...
Page.prototype._fetchCategoriesList = function _fetchCategoriesList() {
    $.ajax({ "url": this._urlFetchCategories, "dataType": "jsonp" })
        .fail(this._fetchCategoriesListFailure)
        .done(this._fetchCategoriesListHandler);
}


Page.prototype._fetchCategoriesListHandler = function _fetchCategoriesListHandler( dataJSON, status, loader ) {
    /// 读取分类成功；
    if ( dataJSON.length ) {
        this._hasLoadCategories = true;
        this.createCategoryModule(dataJSON);
        //this.drawLazyable();
    }
}


Page.prototype._fetchCategoriesListFailure = function _fetchCategoriesListFailure( loader ) {
    /// 读取分类失败；
    this._hasLoadCategories = true;
}
...
Page.prototype.createCategoryModule = function createCategoryModule( categories ) {
    var html = "";
    ...
    this._shopCategories.element.append(html);
    this._updateTimestamp();
    //this._mainContainer.updateFrameSizes();

    /// #测试
    //var c = document.querySelector(".category-content");
    //document.write(window.getComputedStyle(c).display + "<br>" + navigator.userAgent);
}


Page.prototype.createCategoryListModule = function createCategoryListModule( list ) {
    var length = list.length;
    var result = "";

    if ( length == 1 ) {
        /// 1，如果长度为 1，则采用 100.00% 布局。
        ///
        /// |--------------------------|
        /// |         100.00%          |
        /// |--------------------------|

        for ( var i = 0, data = null; i < length; ++i ) {
            data = list[i];
            data.layout = "_1by1";
            data.itemImg = this.getItemImg(data);

            result += template(this._tmplCategoriesToken, data);
        }
    }

    else if ( length == 2 ) {
        /// 2，如果长度能为 2，则使用 50.00% 50.00% 布局。
        /// 
        /// |-------------|------------|
        /// |   50.00%    |   50.00%   |
        /// |-------------|------------|
        ...
        /// 4，如果整除 3 余 1，则采用 50.00% 33.33% 布局。
        ///
        /// |-------------|------------|
        /// |   50.00%    |   50.00%   |
        /// |-------------|------------|
        ...
        /// 5，如果长度整除 3 余 2，则采用 66.66% 33.33 布局。
        ///
        /// |-----------------|--------|
        /// |     66.66%      | 33.33% |
        /// |--------------------------|
...
Page.prototype.getItemImg = function getItemImg( data ) {
    /// 获取指定布局的分类图片文件；
    return data.itemImg[data.layout];
}
